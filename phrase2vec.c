...
int *vocab_hash;
long long vocab_max_size = 10000, vocab_size = 0, phrase_max_size = 100000, phrase_size = 0, layer1_size = 100;
...
int AddWordToSentence(int index, char *word) {
  ...
  if (length > MAX_STRING) length = MAX_STRING;
  phrases[index].words[phrases[index].word_count].word = (char *)calloc(length, sizeof(char));
  strcpy(phrases[index].words[phrases[index].word_count].word, word);
...
int AddSentenceToPhrases(const char *label) {
  ...
  phrases[phrase_size].words = (struct vocab_word *)calloc(MAX_SENTENCE_LENGTH, sizeof(struct vocab_word));
  phrase_size++;
...
void LearnVocabFromTrainFile() {
  ...
  while (1) {
	...
	char path[PATH_MAX];
	snprintf(path, PATH_MAX, "%s%s", train_file, label_name);
	FILE *fin;
...
void *TrainModelThread(void *id) {
  ...
  long long first_phrase;
  first_phrase = phrase_size / (long long) num_threads * (long long)(id);
  ...
  for(i = first_phrase; i < last_phrase; i++) {
    ...
    if (sentence_length == 0) {
      for(j = 0; j < phrases[i].word_count; j++) {
        word = SearchVocab(phrases[i].words[j].word);
        //if (feof(fi)) break;
        if (word == -1) continue;
        word_count++;
        if (word == 0) break;
    ...
    if (dbow) {  //train the dbow architecture
      // in -> hidden
...
void TrainModel() {
  ...
  if (classes == 0) {
    ...
    for (a = 0; a < phrase_size; a++) {
      ...
      for (b = 0; b < count; b++) {
	fprintf(fo, "%s ", phrases[a].words[b].word);
      }
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    printf("\nExamples:\n");
    printf("./phrase2vec -train dir -output vec.txt -debug 2 -size 200 -window 5 -sample 1e-4 -negative 5 -hs 0 -binary 0 -cbow 1\n\n");
    return 0;
  }
  output_file[0] = 0;
  save_vocab_file[0] = 0;
